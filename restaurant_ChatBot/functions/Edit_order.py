...
def edit_order(message,user_id):
    ...
    if food_count != total_numbers:
        ...
        if len(result) != 0:
            ...
            for i, item_size_id in enumerate(item_size_idss):
                query = "SELECT id FROM orders WHERE item_size_id = %s AND cart_id = %s;"
                cursor.execute(query, (item_size_id,cart_id))
                result = cursor.fetchall()
                if len(result) != 0:
                    decrement_query = "UPDATE orders SET quantity = quantity - 1 WHERE cart_id = %s AND item_size_id = %s;"
                    try:
                        cursor.execute(decrement_query, (cart_id, int(item_size_id)))
                        mySqlConnector.commit()

                        print(
                            f"Decrement quantity for cart_id={cart_id}, item_size_id={item_size_id}. New quantity: {cursor.rowcount}.")

                    except Exception as e:
                        print(f"Failed to decrement quantity: {e}")
                else:
                    insert_query = "INSERT INTO orders (cart_id, item_size_id, quantity) VALUES (%s, %s, %s);"
                    try:
                        cursor.execute(insert_query, (cart_id, int(item_size_id),numbers[i]))
                        mySqlConnector.commit()
                        print(f"Inserted new row for cart_id={cart_id}, item_size_id={item_size_id}.")
                    except Exception as e:
